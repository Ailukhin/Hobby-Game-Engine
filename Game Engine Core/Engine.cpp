...
namespace EngineCore
{
		...
		: g_Viewport{ 0 }, pName(_pName), WindowWidth(width), WindowHeight(height)
	{
		...
		// Blend state
		g_d3dBlendStateAlpha = nullptr;
		g_d3dBlendStateOff = nullptr;
	...
	int Engine::InitDirectX(HINSTANCE hInstance, BOOL vSync)
	{
		...
		// Set up blend state ALPHA
		CD3D11_BLEND_DESC BlendState_A;
		ZeroMemory(&BlendState_A, sizeof(CD3D11_BLEND_DESC));

		BlendState_A.RenderTarget[0].BlendEnable = TRUE;
		BlendState_A.RenderTarget[0].SrcBlend = D3D11_BLEND_SRC_ALPHA;
		BlendState_A.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_ALPHA;
		BlendState_A.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;
		BlendState_A.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ZERO;
		BlendState_A.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;
		BlendState_A.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;

		// not using the Alpha channel... not using it on target texel
		BlendState_A.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;

		hr = g_d3dDevice->CreateBlendState(&BlendState_A, &g_d3dBlendStateAlpha);
		assert(SUCCEEDED(hr));
		...
		DirectXDeviceManager::Create(g_d3dDevice, g_d3dDeviceContext);
	...
	{
		ENGINE_UNUSED_VAR(prevInstance);
		ENGINE_UNUSED_VAR(cmdLine);
	...
	void Engine::Cleanup()
	{
		...
		SafeRelease(g_d3dBlendStateAlpha);
		SafeRelease(g_d3dBlendStateOff);
